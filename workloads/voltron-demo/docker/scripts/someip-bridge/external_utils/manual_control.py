...
def find_weather_presets():
    ...
    result.append((clear_noon, "Clear Noon"))

    # Very Foggy Day
    very_foggy_day = carla.WeatherParameters(
        cloudiness=60.0,
        precipitation=10.0,
        sun_altitude_angle=45.0,
        fog_density=99.0,
    ...
    result.append((very_foggy_day, "Very Foggy Day"))

    # Wet Cloudy Night
    wet_cloudy_night = carla.WeatherParameters(
        cloudiness=60.0,
        precipitation=0.0,
        sun_altitude_angle=-90.0,
        fog_density=85.0
    )
    result.append((wet_cloudy_night, "Wet Cloudy Night"))

    # Very Clear Night
    very_clear_night = carla.WeatherParameters(
        cloudiness=5.0,
        precipitation=0.0,
        sun_altitude_angle=-90.0,
        fog_density=0.0
    )
    result.append((very_clear_night, "Very Clear Night"))
...
class World(object):
    ...
    def restart(self):
        ...
        if self.initial_spawn_point is None:
            self.initial_spawn_point = get_initial_spawn_point(self.map)
        ...
        while self.player is None:
            if not self.map.get_spawn_points():
                ...
                sys.exit(1)
            raw_sp = get_initial_spawn_point(self.map)
...
class HUD(object):
    ...
    def tick(self, world, clock):
        ...
        colhist = world.collision_sensor.get_collision_history() if world.collision_sensor is not None else {}
        collision = [colhist[x + self.frame - 200] for x in range(0, 200)] if colhist else [0.0] * 200
        ...
        self._info_text = [
            ...
            'Accelero: (%5.1f,%5.1f,%5.1f)' % (world.imu_sensor.accelerometer) if world.imu_sensor is not None else 'Accelero: N/A',
            'Gyroscop: (%5.1f,%5.1f,%5.1f)' % (world.imu_sensor.gyroscope) if world.imu_sensor is not None else 'Gyroscop: N/A',
            'Location:% 20s' % ('(% 5.1f, % 5.1f)' % (t.location.x, t.location.y)),
            'GNSS:% 24s' % ('(% 2.6f, % 3.6f)' % (world.gnss_sensor.lat, world.gnss_sensor.lon)) if world.gnss_sensor is not None else 'GNSS: N/A',
            'Height:  % 18.0f m' % t.location.z,
            '']
        weather = world.world.get_weather()
        self._info_text += [
            'Weather: %s' % world.weather_preset_name,
            '  Cloudiness: % 5.1f' % weather.cloudiness,
            '  Precipitation: % 5.1f' % weather.precipitation,
            '  Fog Density: % 5.1f' % weather.fog_density,
            '  Sun Altitude: % 5.1f' % weather.sun_altitude_angle,
            '']
